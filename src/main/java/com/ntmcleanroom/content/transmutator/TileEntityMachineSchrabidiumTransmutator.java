...
/**
 * Restored from NTM-CE. Behavior is a straight port of the original machine: burns held energy from
 * a battery/capacitor to convert an input item into its {@link SchrabidiumTransmutatorRecipes} output.
 */
public class TileEntityMachineSchrabidiumTransmutator extends TileEntityMachineBase implements ITickable, IEnergyReceiverMK2 {
    ...
    public boolean isItemValidForSlot(int i, ItemStack stack) {
        switch (i) {
            case 0:
                if (SchrabidiumTransmutatorRecipes.getOutput(stack) != null)
                    return true;
                break;
            case 2:
    ...
    public boolean canProcess() {
        ...
        if (inventory.getStackInSlot(0) == null || inventory.getStackInSlot(0).isEmpty())
            return false;
        long recipePower = SchrabidiumTransmutatorRecipes.getEnergy(inventory.getStackInSlot(0));
        ...
        ItemStack outputItem = SchrabidiumTransmutatorRecipes.getOutput(inventory.getStackInSlot(0));
    ...
    public void process() {
        ...
        if (process >= processSpeed) {
            ...
            power -= SchrabidiumTransmutatorRecipes.getEnergy(inventory.getStackInSlot(0));
            if (power < 0)
                power = 0;
            process = 0;

            if (inventory.getStackInSlot(1).isEmpty()) {
                inventory.setStackInSlot(1, SchrabidiumTransmutatorRecipes.getOutput(inventory.getStackInSlot(0)).copy());
            } else {
